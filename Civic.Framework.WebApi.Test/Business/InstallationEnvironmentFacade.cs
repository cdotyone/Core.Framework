...
namespace Civic.Framework.WebApi.Test.Business
{
    ...
    public partial class InstallationEnvironmentFacade : IEntityBusinessFacade<IExampleInstallationEnvironment>
    {
            private readonly IEntityRepository<IExampleInstallationEnvironment> _repository;
            private readonly IEntityEventHandlerFactory _handlers;
    		private readonly IEntityCreateFactory _factory;
    
            public InstallationEnvironmentFacade(IEntityRepository<IExampleInstallationEnvironment> repository, IEntityCreateFactory factory, IEntityEventHandlerFactory handlers)
            {
        	...
        	{
                using(Logger.CreateTrace(LoggingBoundaries.ServiceBoundary, typeof(InstallationEnvironmentFacade), "Get")) {
        	...
        	{
                using(Logger.CreateTrace(LoggingBoundaries.ServiceBoundary, typeof(InstallationEnvironmentFacade), "GetPaged")) {
        	...
        	public void Save(IEntityRequestContext context, IExampleInstallationEnvironment entity) 
        	{
                using(Logger.CreateTrace(LoggingBoundaries.ServiceBoundary, typeof(InstallationEnvironmentFacade), "Save")) {
        	...
        	{
                using(Logger.CreateTrace(LoggingBoundaries.ServiceBoundary, typeof(InstallationEnvironmentFacade), "Remove")) {
                    var first = context.Operations.Count == 0;
            ...
            public IEnumerable<IExampleInstallationEnvironment> GetPaged(ClaimsPrincipal who, IEntityInfo info, int skip, ref int count, bool retCount, string filterBy, string orderBy)
            {
                return GetPaged(new EntityRequestContext { Who = who }, info, skip, ref count, retCount, filterBy, orderBy);
            }
        
        	public IExampleInstallationEnvironment Get(ClaimsPrincipal who, IExampleInstallationEnvironment entity) {
        		return Get(new EntityRequestContext {Who = who}, entity);
        	}
        
        	public void Save(ClaimsPrincipal who, IExampleInstallationEnvironment entity) {
        		Save(new EntityRequestContext {Who = who}, entity);
        	}
        
        	public void Remove(ClaimsPrincipal who, IExampleInstallationEnvironment entity ) {
